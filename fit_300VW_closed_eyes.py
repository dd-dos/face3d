...
if __name__=='__main__':
    shutil.rmtree(f'300VW-3D_cropped_closed_eyes_3ddfa', ignore_errors=True)
    os.makedirs(f'300VW-3D_cropped_closed_eyes_3ddfa', exist_ok=True)
    for folder_path in glob.glob('300VW-3D_cropped_closed_eyes/*'):
        folder_img_name = folder_path.split('/')[-1]
        os.makedirs(
            os.path.join(f'300VW-3D_cropped_closed_eyes_3ddfa', folder_img_name),
            exist_ok=True
        )
