...
def task(item):
    ...
    if utils.check_eye_status(eyes_2d['left']) == 'closed' and utils.check_eye_status(eyes_2d['right']) == 'closed':
        ...
        sio.savemat(f'300VW-3D_cropped_closed_eyes/{folder_id}/{file_id}.mat', {'pt3d': pt3d})
    elif utils.check_eye_status(eyes_2d['left']) == 'opened' and utils.check_eye_status(eyes_2d['right']) == 'opened':
        shutil.copyfile(item.replace('mat', 'jpg'), f'300VW-3D_cropped_opened_eyes/{folder_id}/{file_id}.jpg')
        pt3d = sio.loadmat(item)['pt3d']
        pt3d = utils.replace_eyes(pt2d, pt3d)
        sio.savemat(f'300VW-3D_cropped_opened_eyes/{folder_id}/{file_id}.mat', {'pt3d': pt3d})
    elif utils.check_eye_status(eyes_2d['left']) == 'semi' and utils.check_eye_status(eyes_2d['right']) == 'semi':
        shutil.copyfile(item.replace('mat', 'jpg'), f'300VW-3D_cropped_semi_eyes/{folder_id}/{file_id}.jpg')
        pt3d = sio.loadmat(item)['pt3d']
        pt3d = utils.replace_eyes(pt2d, pt3d) 
        sio.savemat(f'300VW-3D_cropped_semi_eyes/{folder_id}/{file_id}.mat', {'pt3d': pt3d})

if __name__=='__main__':
    shutil.rmtree('300VW-3D_cropped_closed_eyes', ignore_errors=True)
    shutil.rmtree('300VW-3D_cropped_opened_eyes', ignore_errors=True)
    shutil.rmtree('300VW-3D_cropped_semi_eyes', ignore_errors=True)
    shutil.rmtree('test_images', ignore_errors=True)

    os.makedirs('300VW-3D_cropped_closed_eyes', exist_ok=True)
    os.makedirs('300VW-3D_cropped_opened_eyes', exist_ok=True)
    os.makedirs('300VW-3D_cropped_semi_eyes', exist_ok=True)
    ...
    for folder_path in glob.glob('../300VW-3D_cropped/*'):
        ...
        os.makedirs(
            os.path.join('300VW-3D_cropped_opened_eyes', folder_name),
            exist_ok=True
        )
        os.makedirs(
            os.path.join('300VW-3D_cropped_semi_eyes', folder_name),
            exist_ok=True
        )
        os.makedirs(
